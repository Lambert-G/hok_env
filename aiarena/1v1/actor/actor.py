...
class Actor:
    ...
    def _print_info(self, game_id, game_info, episode_infos, eval, common_ai=None):
        ...
        for i, agent in enumerate(self.agents):
            ...
            print(
                f"[actor] episode end: i={i}, keep_latest={agent.keep_latest}, eval={eval}, actor_id={self.m_config_id}")
            print(
                f"[actor] reward={episode_infos[i].get('reward')}, win={episode_infos[i].get('win')}, len={game_info.get('length')}")

            if agent.keep_latest and eval:
                payload = {
                        "role": "actor",
                        "actor_id": self.m_config_id,
                        "reward": float(to_builtin(episode_infos[i]["reward"])),
                        "win": int(to_builtin(episode_infos[i]["win"])),
                        "hurt_per_frame": float(to_builtin(episode_infos[i]["hurt_per_frame"])),
                        "money_per_frame": float(to_builtin(episode_infos[i]["money_per_frame"])),
                        "totalHurtToHero": float(to_builtin(episode_infos[i]["totalHurtToHero"])),
                        "kill": int(to_builtin(episode_infos[i]["kill"])),
                        "death": int(to_builtin(episode_infos[i]["death"])),
                        "assistCnt": int(to_builtin(episode_infos[i]["assistCnt"])),
                        "hurtH_per_frame": float(to_builtin(episode_infos[i]["hurtH_per_frame"])),
                        "hurtBH_per_frame": float(to_builtin(episode_infos[i]["hurtBH_per_frame"])),
                        "length": game_info["length"],
                    }
                print("[actor] going to upload:", {k: payload[k] for k in ("actor_id", "reward", "win", "length")})
                self.upload_monitor_data(payload)
                print("[actor] upload queued.")
            else:
                print("[actor] NOT uploading this episode (condition not met).")
